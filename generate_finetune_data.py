...
def parse_spec(html_path):
    ...
    for h2 in soup.find_all("h2"):
        ...
        if len(entries) >= MAX_SAMPLES: break
    return entries

def parse_tests():
    samples = []
    for fname in os.listdir(TESTS_DIR):
        if not fname.endswith(".pine"): continue
...
def main():
    ...
    with open("training_dataset.jsonl","w",encoding="utf-8") as t:
        for s in train: t.write(json.dumps(s,ensure_ascii=False)+"\n")
    with open("validation_dataset.jsonl","w",encoding="utf-8") as v:
        for s in val:   v.write(json.dumps(s,ensure_ascii=False)+"\n")
    print(f"[✔] Wrote {len(train)} train and {len(val)} validation samples")
